...
class ArticleModel extends Model{
    ...
    public function search($map,$page) {
        ...
        ->field('zj_article.id,title,sortid,vote,click,time')
        ->order('id DESC')
        ->select();
        return $list;
    }
...
/*    	$res = $this->checkDocumentType( I('type'), I('pid') );
        ...
        //内容添加或更新完成
        file_put_contents('modle.txt',json_encode($data));
        return $data;
    }
...
public function getArticle($where,$lenght=1, $order = 'id DESC'){
    $data=M('Article')->where($where)->field('id,time,title,desc,content,up,down,vote')->limit($lenght)->order($order)->select();
    return $data;
}
}
